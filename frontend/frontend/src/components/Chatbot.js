...
const Chatbot = () => {
  ...
  const [pageLabel, setPageLabel] = useState(location.pathname); // use path as key
  const [displayLabel, setDisplayLabel] = useState(getPageLabel(location.pathname)); // human readable

  useEffect(() => {
    setPage(location.pathname);
    setPageLabel(location.pathname);
    setDisplayLabel(getPageLabel(location.pathname));
  }, [location.pathname]);
  ...
  const handleSend = async (e) => {
    ...
    try {
      ...
      console.error(err);
      setMessages([...newMessages, { from: 'bot', text: 'Sorry, there was an error reaching the server.' }]);
    }
  };

  const handleReset = () => {
    setMessages([
      { from: 'bot', text: `Hi! I can help explain the docs for "${displayLabel}". Ask me anything.` }
    ]);
  };
        ...
        >
          ...
          <div style={{ flex: 1, padding: '1rem', overflowY: 'auto', background: '#f9f9f9' }}>
            <div style={{ fontSize: 13, color: '#888', marginBottom: 10 }}>
              <span>Context: <b>{displayLabel}</b></span>
            </div>
            {messages.map((msg, i) => (
          ...
          <form onSubmit={handleSend} style={{ display: 'flex', borderTop: '1px solid #eee', padding: '0.5rem' }}>
            <input
              ...
              placeholder={`Ask about ${displayLabel}...`}
              style={{
                flex: 1,
                border: 'none',
                outline: 'none',
